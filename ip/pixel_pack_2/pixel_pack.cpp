...
#pragma HLS pipeline II=2
			...
			for (int i = 0; i < 2; ++i) {
				if (!last) {
					user |= stream_in_48->user;
					last = stream_in_48->last;
					ap_uint<48> in_data = stream_in_48->data;
					ap_uint<9> out_c1 =
						ap_uint<9>(in_data.range(15,8)) + ap_uint<9>(in_data.range(39,32));
					ap_uint<9> out_c2 =
						ap_uint<9>(in_data.range(23,16)) + ap_uint<9>(in_data.range(47,40));
					data.range(32*i + 7, 32*i + 0) = in_data.range(7,0);
					data.range(32*i+ 15, 32*i + 8) = out_c1.range(8,1);
					data.range(32*i + 23,32*i + 16) = in_data.range(31,24);
					data.range(32*i + 31,32*i + 24) = out_c2.range(8,1);
					++stream_in_48;
				}
			}
			if (!delayed_last) {
...
#pragma HLS pipeline II=2
			...
			for (int i = 0; i < 2; ++i) {
				if (!last) {
					...
					user |= stream_in_48->user;
					last = stream_in_48->last;
					in_data = stream_in_48->data;
					ap_uint<9> out_c1 =
						ap_uint<9>(in_data.range(15,8)) + ap_uint<9>(in_data.range(39,32));
					ap_uint<9> out_c2 =
						ap_uint<9>(in_data.range(23,16)) + ap_uint<9>(in_data.range(47,40));
					data.range(32*i + 7, 32*i + 0) = out_c1.range(8,1);
					data.range(32*i+ 15, 32*i + 8) = in_data.range(7,0);
					data.range(32*i + 23,32*i + 16) = out_c2.range(8,1);
					data.range(32*i + 31,32*i + 24) = in_data.range(31,24);
					++stream_in_48;
				}
			}
			if (!delayed_last) {

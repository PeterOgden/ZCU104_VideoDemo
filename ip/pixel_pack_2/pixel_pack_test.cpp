...
int main() {
	...
	reset_data();
	pixel_pack(input_data, output_data, 3, 0);
	for (int i = 0; i < 12; ++i) {
		assert(output_data[i].user == (i == 0? 1: 0));
		assert(output_data[i].last == (i == 11? 1: 0));
		assert(output_data[i].data.range(7,0) == i*12);
		assert(output_data[i].data.range(15,8) == i*12 + 1);
		assert(output_data[i].data.range(23,16) == i*12 + 3);
		assert(output_data[i].data.range(31,24) == i*12 + 4);
		assert(output_data[i].data.range(39,32) == i*12 + 6);
		assert(output_data[i].data.range(47,40) == i*12 + 7);
		assert(output_data[i].data.range(55,48) == i*12 + 9);
		assert(output_data[i].data.range(63,56) == i*12 + 10);
	}
	reset_data();
	pixel_pack(input_data, output_data, 4, 0);
	for (int i = 0; i < 12; ++i) {
		assert(output_data[i].user == (i == 0? 1: 0));
		assert(output_data[i].last == (i == 11? 1: 0));
		std::cout << std::hex << output_data[i].data << std::endl;
	}
}
